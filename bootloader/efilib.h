...
// ==========================================
BOOLEAN
addIntToBuf(UINTN number, UINT8 base, BOOLEAN signed_num, UINTN min_digits, char *buf, 
               UINTN *buf_idx) {
...
// ========================================================================
bool formatString(char *buf, char *fmt, va_list args) {
    bool result = true;
    char charstr[2] = {0};    
    UINTN buf_idx = 0;
    ...
    for (UINTN i = 0; fmt[i] != u'\0'; i++) {
        if (fmt[i] == u'%') {
            ...
            if (int_num) {
                ...
                addIntToBuf(number, base, signed_num, precision, buf, &buf_idx);
            }
            ...
            if (double_num) {
                ...
                // Add digits to write buffer
                addIntToBuf(number, base, signed_num, num_digits, buf, &buf_idx);
...
bool sprintf(char *s, char *fmt, ...) {
    va_list args;
    va_start(args, fmt);
    return formatString(s, fmt, args);
}
...
// =================
BOOLEAN getNumber(UINTN *number) {
  EFI_INPUT_KEY key = {0};
